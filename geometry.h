...
template <typename T> struct vec<2, T> {
    ...
    inline vec<2, T> operator +(const vec<2, T>& V) const { return vec<2, T>(x + V.x, y + V.y); }
    inline vec<2, T> operator -(const vec<2, T>& V) const { return vec<2, T>(x - V.x, y - V.y); }
    inline vec<2, T> operator *(float f)          const { return vec<2, T>(x * f, y * f); }
...
template <typename T> struct vec<3, T> {
    ...
    inline T       operator *(const vec<3, T >& v) const { return x * v.x + y * v.y + z * v.z; }
